...
export const RelationshipDetail = () => {
  ...
  const { getFavorsForRelationship, getRelationshipBalance } = useFavorsEnhanced();
  ...
  const getBalanceStatus = () => {
    if (balance > 2) return { label: "You're giving more", color: "text-blue-600", bg: "bg-blue-50" };
    if (balance < -2) return { label: "They're giving more", color: "text-orange-600", bg: "bg-orange-50" };
    return { label: "Balanced relationship", color: "text-green-600", bg: "bg-green-50" };
  };
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              ...
              <Badge className={getImportanceColor(relationship.importance_level || 3)}>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-semibold text-gray-900 mb-1">Relationship Balance</h3>
                <p className={`text-sm ${balanceStatus.color}`}>{balanceStatus.label}</p>
              </div>
              <div className={`px-4 py-2 rounded-lg ${balanceStatus.bg}`}>
                <div className="text-center">
                  <div className="text-2xl font-bold text-gray-900">{balance > 0 ? `+${balance}` : balance}</div>
            ...
            <div className="grid grid-cols-2 gap-4 mt-4">
              <div className="text-center p-3 bg-green-50 rounded-lg">
                <div className="text-xl font-bold text-green-600">{givenFavors.length}</div>
                <div className="text-xs text-gray-600">Given</div>
              </div>
              <div className="text-center p-3 bg-blue-50 rounded-lg">
                <div className="text-xl font-bold text-blue-600">{receivedFavors.length}</div>
          ...
          <TabsContent value="history" className="space-y-4">
            {favors.length === 0 ? (
              <Card>
                <CardContent className="pt-6">
                  <div className="text-center text-gray-500 py-8">
                    ...
                    <p className="text-sm">Start by adding your first favor with {relationship.name}.</p>
                  </div>
                </CardContent>
              </Card>
            ) : (
              <div className="space-y-3">
                {favors.map((favor) => (
                  <Card key={favor.id} className="hover:shadow-sm transition-shadow">
                    <CardContent className="pt-4">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            <Badge 
                              variant={favor.direction === "given" ? "default" : "secondary"}
                              className="text-xs"
                            >
                          ...
                          <p className="text-sm text-gray-900 mb-1">{favor.description}</p>
                ...
                <CardContent className="pt-6">
                  <div className="text-center text-gray-500 py-8">
                    ...
                    <p className="text-sm mb-4">
                      Click "AI Insights" to get personalized relationship recommendations.
                    </p>
                    <Button 
                      onClick={handleGenerateRecommendations}
                      disabled={loadingRecommendations}
                  ...
                  <Card key={index} className="hover:shadow-sm transition-shadow">
                    <CardHeader>
                      <div className="flex items-start justify-between">
                        ...
                        <Badge 
                          variant={rec.priority_level >= 4 ? "destructive" : rec.priority_level >= 3 ? "default" : "secondary"}
                        >
                          {rec.priority_level >= 4 ? "High" : rec.priority_level >= 3 ? "Medium" : "Low"} Priority
                        </Badge>
                      </div>
                    </CardHeader>
                    <CardContent>
                      ...
                      {rec.suggested_actions?.how_to_execute && (
                        <div className="space-y-2">
                          <h4 className="font-semibold text-sm text-gray-900">How to execute:</h4>
                          <ul className="text-sm text-gray-600 space-y-1">
                            {rec.suggested_actions.how_to_execute.map((action: string, i: number) => (
